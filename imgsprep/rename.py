...
content.append(("branchclaw/fear-012", "panic"))
# Sea Monsters exists once in its old for in B&C and once in its new form in JE
content = [line for line in content if not (line[0].startswith("branchclaw") and "sea_monsters" in line[1])]
...
for card in content:
    ...
    if powerrating >= 69:
        ...
        print("NOT MATCHED: {:40} {:>30}({}) {:>20}({}) {:>20}({}) {:>20}".format(ocr, power, powerrating, fear, fearrating, event, eventrating, file))
if len(powers) > 0:
